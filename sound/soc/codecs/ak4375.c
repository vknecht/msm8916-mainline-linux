...
static const struct reg_sequence ak4375_reg_bclk[] = {
	{ AK4375_03_POWER_MANAGEMENT4, 0x50 },
	{ AK4375_04_OUTPUT_MODE_SETTING, 0x14 },
	{ AK4375_05_CLOCK_MODE_SELECT, 0x0a },
	{ AK4375_06_DIGITAL_FILTER_SELECT, 0x00 },
	{ AK4375_07_DAC_MONO_MIXING, 0x21 },
	{ AK4375_08_JITTER_CLEANER_SETTING1, 0x00 },
	{ AK4375_09_JITTER_CLEANER_SETTING2, 0x00 },
	{ AK4375_0A_JITTER_CLEANER_SETTING3, 0x00 },
	{ AK4375_0B_LCH_OUTPUT_VOLUME, 0x19 },
	{ AK4375_0C_RCH_OUTPUT_VOLUME, 0x19 },
	{ AK4375_0D_HP_VOLUME_CONTROL, 0x73 },
	{ AK4375_0E_PLL_CLK_SOURCE_SELECT, 0x01 },
	{ AK4375_0F_PLL_REF_CLK_DIVIDER1, 0x00 },
	{ AK4375_10_PLL_REF_CLK_DIVIDER2, 0x00 },
	{ AK4375_11_PLL_FB_CLK_DIVIDER1, 0x00 },
	{ AK4375_12_PLL_FB_CLK_DIVIDER2, 0x4f },
	{ AK4375_13_SRC_CLK_SOURCE, 0x01 },
	{ AK4375_14_DAC_CLK_DIVIDER, 0x09 },
	{ AK4375_15_AUDIO_IF_FORMAT, 0x01 },
	{ AK4375_24_MODE_CONTROL, 0x00 },
};
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.enumerated.item[0] = ak4375->hp_state;
	dev_info(ak4375->dev, "get_akpwdn = %d\n", ak4375->hp_state);
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (value > 0) {
		...
		mdelay(1);
		snd_soc_component_update_bits(component, AK4375_02_POWER_MANAGEMENT3, 0x01, 0x01); //PMDA=1
		snd_soc_component_update_bits(component, AK4375_01_POWER_MANAGEMENT2, 0x02, 0x02); //PMCP2=1
		snd_soc_component_update_bits(component, AK4375_03_POWER_MANAGEMENT4, 0x03, 0x03);
	} else {
		snd_soc_component_update_bits(component, AK4375_03_POWER_MANAGEMENT4, 0x03, 0x00);
		snd_soc_component_update_bits(component, AK4375_01_POWER_MANAGEMENT2, 0x02, 0x00); //PMCP2=0
		snd_soc_component_update_bits(component, AK4375_02_POWER_MANAGEMENT3, 0x01, 0x00); //PMDA=0
...
static int ak4375_set_pllblock(struct snd_soc_component *component, int fs)
{
	...
	ret = snd_soc_component_write(component, AK4375_05_CLOCK_MODE_SELECT, mode);
	if (ret < 0) {
		dev_err(component->dev, "Failed writing AK4375_05_CLOCK_MODE_SELECT\n");
		return ret;
	}
	...
	MDIVbit--;
	/* PLD 15-0 bits */
	ret = snd_soc_component_write(component, AK4375_0F_PLL_REF_CLK_DIVIDER1, ((PLDbit & 0xFF00) >> 8));
	if (ret < 0) {
		dev_err(component->dev, "Failed writing AK4375_0F_PLL_REF_CLK_DIVIDER1\n");
		return ret;
	}

	ret = snd_soc_component_write(component, AK4375_10_PLL_REF_CLK_DIVIDER2, ((PLDbit & 0x00FF) >> 0));
	if (ret < 0) {
		dev_err(component->dev, "Failed writing AK4375_10_PLL_REF_CLK_DIVIDER2\n");
		return ret;
	}

	/* PLM 15-0 bits */
	ret = snd_soc_component_write(component, AK4375_11_PLL_FB_CLK_DIVIDER1, ((PLMbit & 0xFF00) >> 8));
	if (ret < 0) {
		dev_err(component->dev, "Failed writing AK4375_11_PLL_FB_CLK_DIVIDER1\n");
		return ret;
	}

	ret = snd_soc_component_write(component, AK4375_12_PLL_FB_CLK_DIVIDER2, ((PLMbit & 0x00FF) >> 0));
	if (ret < 0) {
		dev_err(component->dev, "Failed writing AK4375_12_PLL_FB_CLK_DIVIDER2\n");
		return ret;
	}
	...
	ret = snd_soc_component_write(component, AK4375_13_SRC_CLK_SOURCE, (nTemp | 0x01));
	if (ret < 0) {
		dev_err(component->dev, "Failed writing AK4375_13_SRC_CLK_SOURCE\n");
		return ret;
	}

	ret = snd_soc_component_update_bits(component, AK4375_0E_PLL_CLK_SOURCE_SELECT, 0x1, 0x1); /* PLS=1(BICK) */
	if (ret < 0) {
		dev_err(component->dev, "Failed updating AK4375_0E_PLL_CLK_SOURCE_SELECT\n");
		return ret;
	}

	ret = snd_soc_component_write(component, AK4375_14_DAC_CLK_DIVIDER, MDIVbit); /* MDIV 7-0 bits */
	if (ret < 0) {
		dev_err(component->dev, "Failed writing AK4375_14_DAC_CLK_DIVIDER\n");
		return ret;
	}
			    ...
			    struct snd_soc_dai *dai)
{
	...
	ret = snd_soc_component_write(component, AK4375_05_CLOCK_MODE_SELECT, fs);
	if (ret < 0) {
		dev_err(component->dev, "Failed writing to AK4375_05_CLOCK_MODE_SELECT\n");
		return ret;
	}
...
static void ak4375_power_off(struct ak4375_priv *ak4375)
{
	...
	gpiod_set_value_cansleep(ak4375->ldo_gpiod, 0);
...
static void ak4375_power_on(struct ak4375_priv *ak4375)
{
	...
	gpiod_set_value_cansleep(ak4375->ldo_gpiod, 1);
	usleep_range(3000, 4000);
...
static int ak4375_pre_init(struct snd_soc_component *component)
{
	struct ak4375_priv *ak4375 = snd_soc_component_get_drvdata(component);
	int ret;
	...
	dev_info(ak4375->dev, "BCLK MODE INIT\n");

	ret = regmap_multi_reg_write(ak4375->regmap, ak4375_reg_bclk, ARRAY_SIZE(ak4375_reg_bclk));
	if (ret < 0) {
		dev_err(component->dev, "Failed to write bclk register sequence\n");
		return ret;
	}
...
static int ak4375_i2c_probe(struct i2c_client *i2c)
{
	...
	ak4375->ldo_gpiod = devm_gpiod_get_optional(ak4375->dev, "ldo", GPIOD_OUT_LOW);
	if (IS_ERR(ak4375->ldo_gpiod))
		return PTR_ERR(ak4375->ldo_gpiod);
