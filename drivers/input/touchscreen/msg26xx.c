...
struct touch_event {
	u8	mode;
	struct packet packet[MAX_SUPPORTED_FINGER_NUM];
	u8	proximity;
	u8	checksum;
};
...
static int mstar_init_regulators(struct msg26xx_ts_data *msg26xx)
{
	...
	msg26xx->supplies[1].supply = "vddo";
				...
				const struct point_coord *p)
{
	...
	touchscreen_report_pos(msg26xx->input_dev, &msg26xx->prop, p->x, p->y, true);
...
static irqreturn_t mstar_ts_irq_handler(int irq, void *msg26xx_handler)
{
	...
	struct point_coord point_coord;
	...
	for (i = 0; i < MAX_SUPPORTED_FINGER_NUM; i++) {
		p = &touch_event.packet[i];
		/* Ignore non-pressed finger data */
		if (p->xy == 0xFF && p->x == 0xFF && p->y == 0xFF)
			continue;

		point_coord.x = (((p->xy & 0xF0) << 4) | p->x) * msg26xx->prop.max_x / TPD_WIDTH;
		point_coord.y = (((p->xy & 0x0F) << 8) | p->y) * msg26xx->prop.max_y / TPD_HEIGHT;
		mstar_report_finger(msg26xx, i, &point_coord);
	}
